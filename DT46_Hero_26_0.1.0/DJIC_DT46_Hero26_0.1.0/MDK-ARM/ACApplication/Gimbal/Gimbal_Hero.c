...
void Gimbal_SendCmd(void)
{
    ...
    // 获取当前系统时间(ms)
    uint32_t now_time = GET_SYSTEM_TICK();
...
static void Gimbal_Update_Target(void)
{
    ...
    Gimbal_Instance.Calc.Yaw.T_Angle = Gimbal_Instance.Calc.Yaw.T_Angle
                                        - MyMath_Map_Range(AddYaw,-CMD_CTRL_RANGE,CMD_CTRL_RANGE,-GIMBAL_MAX_ANGLE_STEP_DEG,GIMBAL_MAX_ANGLE_STEP_DEG);
    //限幅角度-180 ~ 180 循环模式
    ...
    Gimbal_Instance.Calc.Pitch.T_Angle = Gimbal_Instance.Calc.Pitch.T_Angle
                                        - MyMath_Map_Range(AddPitch,-CMD_CTRL_RANGE,CMD_CTRL_RANGE,-GIMBAL_MAX_ANGLE_STEP_DEG,GIMBAL_MAX_ANGLE_STEP_DEG);
    //限幅俯仰角，非循环模式

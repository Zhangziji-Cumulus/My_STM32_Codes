  ...
  { 
		...
		if (DBT_RX.B2.RC_Ctl.Flag.frameLost)
     {
		...
		{	
			...
			Motor_DJI_Chassis(DJI_MFeedback_CAN2,&hfdcan2,1.0f,Chassis_Mec.FL.T_rpm,Chassis_Mec.FR.T_rpm,Chassis_Mec.BL.T_rpm,Chassis_Mec.BR.T_rpm);	
		}
//==============================================================//
...
__attribute__((used)) void HEROShootingTask(void *argument)
{
  for(;;)
  {
		...
		if (DBT_RX.B2.RC_Ctl.Flag.frameLost)
     {
				...
				if(timecount < 150)
				{
					DJI_MOTOR_EmergencySTOP_ALL(DJI_MFeedback_CAN1,&hfdcan1,10.0f);
					timecount++;
				}
		 }
		else if(g_motor_run_enable == 0)
		...
		else
		{
			...
			Motor_DJI_ShootingFri(DJI_MFeedback_CAN1,&hfdcan1,1.0f,SV.UP_Lrpm,SV.UP_Rrpm,SV.Dowm_Mrpm);	
		}
  ...
  for(;;)
  {
		...
		if (DBT_RX.B2.RC_Ctl.Flag.frameLost)
    {
		...
		else
		{
			...
			{		timecount = 0;
				if(abs(DJI_MFeedback_CAN1[0].current_ma) < 2000)
				{
					Motor_DJI_Dial(DJI_MFeedback_CAN1,&hfdcan1,1,200);
				}
			}
			else
			{
